...
<body class="bg-slate-100">
    <div class="border border-1 bg-white rounded-xl p-4 w-1/4 mt-10 m-auto">
        ...
        <h1 class="font-bold text-xl text-teal-500 text-center mb-2">Employee Register Here</h1>
        ...
        <form class="flex flex-col mb-5" action="{{ route('employee.processRegister') }}" method="post">
            @csrf
            
            <label for="name" class="mt-4">Name</label>
            <input class="p-4 invalid border mt-2 bg-slate-50 rounded-xl @error('name')  border-red-500 @enderror"
            ...
            <input class="p-4 mt-4 bg-teal-500 text-white font-bold rounded-lg" type="submit" value="Register">

        </form>

        <a href="{{ route('employee.login') }}" class="mt-4 font-semibold text-center">Login Employee</a>
    </div>
