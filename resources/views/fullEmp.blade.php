        ...
        <div class="p-2 m-2 bg-teal-50 rounded-xl shadow-xl w-full flex gap-2">
            ...
            <div class="w-2/3 font-medium p-4">
                ...
                <div class="flex gap-2 mt-5">
                    <a id="edit_button" class="bg-teal-500 w-24 text-white text-center font-bold p-2 rounded-xl"
                        href="{{ url('/employee/edit/' . $employee->id) }}">Edit</a>

                    <form action="{{ url('/employee/delete/' . $employee->id) }}" method="post">
                        @csrf
                        @method('DELETE')
                        <input class="bg-teal-100 w-24 text-teal-500 font-bold p-2 rounded-xl" type="submit" value="Delete">
                    </form>
                </div>
            </div>
        </div>
